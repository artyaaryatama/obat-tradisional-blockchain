    ...
    const loadData = async () => {
      if (contract && userData.instanceName) {
        try {
          ...
          console.log(tx);
          const [jenisSediaanArray, factoryNameArray, statusArray, latestTimestampArray, idArray] = tx;
          ...
          const reconstructedData = jenisSediaanArray.map((jenisSediaan, index) => {
            ...
            const formattedTimestamp = timestampDate.toLocaleDateString('en-US', options);
  ...
  const getDetailCpotb = async (id) => {
    ...
    try {
      ...
      const detailCpotb = {
        ...
        timestampRequest: new Date(Number(tx.timestampRequest) * 1000).toLocaleDateString('en-US', options), 
        timestampApprove: Number(tx.timestampApprove) > 0 ? new Date(Number(tx.timestampApprove) * 1000).toLocaleDateString('en-US', options): "-",
